...
public final class McpResourceTokenPostProcessor implements TokenPostProcessor {
  ...
  public void process(TokenPostProcessorContext context) {
    ...
    if (!ResourceBinding.protects(client)) {
      return;
    }
    ...
    Object proof = session.removeAttribute(ResourceBinding.REQUEST_VERIFIED);
    if ((!OAuth2Constants.AUTHORIZATION_CODE.equals(grant)
            && !OAuth2Constants.REFRESH_TOKEN.equals(grant))
        || !(proof instanceof ResourceBinding.Verification verification)
        || !verification.matches(client, grant, canonical)
