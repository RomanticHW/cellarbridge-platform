...
@Component
final class SettlementCursorCodec {
  private static final Base64.Encoder ENCODER = Base64.getUrlEncoder().withoutPadding();
  private static final Base64.Decoder DECODER = Base64.getUrlDecoder();
  private final byte[] secret;

  SettlementCursorCodec(@Value("${cellarbridge.settlement.cursor-secret}") String secret) {
    if (secret == null || secret.length() < 32) {
      throw new IllegalArgumentException(
          "Settlement cursor secret must contain at least 32 characters");
    }
  ...
  static String filterHash(String canonical) {
    try {
      return HexFormat.of()
          .formatHex(
  ...
  private byte[] sign(String value) {
    try {
      ...
      throw new IllegalStateException("Settlement cursor signing is unavailable", exception);
    }
  }

  private static SettlementProblem invalid() {
    return new SettlementProblem("VALIDATION_FAILED", "Cursor is invalid or stale");
  }
}
