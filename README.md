...
> **Project status — Atomic inventory reservation execution in review.** Current/Legacy order events now drive deterministic FIXED/AUTO allocation, savepoint-safe all-or-nothing Lot updates, immutable conflict evidence, and reliable reservation outcomes. Trade Order outcome consumption and Reservation API/UI remain later slices.
...
flowchart LR
    ...
    G --> H[Atomic inventory reservation<br/>In review]
...
| Inventory reservation | Uses atomic conditional updates and deterministic allocation to prevent overselling | Implemented in review |
...
**Concurrency demonstrated through reservation execution.** Task 08 B1 combines deterministic candidate locking with exact PostgreSQL conditional updates. Controlled contention proves no oversell; multi-line failure proves savepoint rollback, and the outer transaction preserves only the failed Reservation/Attempt/outcome evidence. Trade Order outcome consumption remains a later slice.
...
Partner-scoped `/buyer/orders*` resources are also implemented; reservation API/UI, order outcome
consumption, and later business paths remain design contracts.
...
在目标场景中，销售人员为商业客户选择多个酒款 SKU，并生成客户专属报价。系统根据客户资格、货源位置、目标交付时间、交付区域和成本构成评估可用的贸易交付路径；每个候选方案都给出可解释的通过、拒绝原因和评分。当前可执行核心继续覆盖审批、客户接受、唯一订单创建和 Inventory 原子预留执行。订单 outcome 消费、履约、异常、应收和完整审计证据属于后续已设计阶段。
...
| 库存预占 | 使用原子条件更新和确定性分配防止超卖 | 已实现，评审中 |
...
**并发正确性已在预留执行链路获得证明。** Task 08 B1 将确定性候选锁定与 PostgreSQL 精确条件更新组合起来；受控并发证明不会超卖，多行失败证明 savepoint 会回滚全部数量写入，外层事务只保留失败的 Reservation/Attempt/outcome 证据。Trade Order 消费 outcome 仍属于后续切片。
...
仍是权威契约；`/me`、`/partners*`、`/catalog/skus*`、`/quotations*` 与客户安全报价查看/接受/拒绝均已实现。内部 `/orders*` 和 Partner-scoped `/buyer/orders*` 也已实现；预留 API/UI、订单 outcome 消费及后续业务路径仍为设计契约。
