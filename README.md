...
> **Project status — Observable and security-hardened demonstration available.** The executable flow now continues through settlement and reporting with traceable local events, operational metrics, a reproducible observability profile, and supply-chain security gates.
...
CellarBridge（酒桥）模拟一家进口酒饮供应链服务商的目标企业流程。它不是面向消费者的酒类商城，也不是把若干 CRUD 页面拼在一起的后台模板。当前可运行闭环已从库存预占、路线绑定履约和异常恢复继续延伸到应收、审计与经营报表，并提供可追踪本地事件、运营指标、可复现观测环境和供应链安全门禁。
