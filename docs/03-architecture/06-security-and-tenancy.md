...
- `/mcp` 是与 REST API 同进程、独立 filter chain 的 OAuth 2.0 Resource Server，复用 issuer/
  JWK，但只接受专用 canonical MCP resource/audience、`mcp:read` scope 和允许的 host client；
- RFC 9728 metadata/challenge 支持标准发现；Agent Host 通过预登记 public client、Authorization
  Code + S256 PKCE 获取令牌，仓库内 Keycloak provider 强制 authorization/token/refresh 的
  RFC 8707 resource 绑定；不支持动态注册、token exchange 或 credential forwarding；
...
- MCP 还校验独立 Host allowlist、chunked/Content-Length 请求体上限、受控 client
  token-bucket、全局/每 client bulkhead、请求超时和 PostgreSQL `SET LOCAL statement_timeout`；
...
- 依赖和容器扫描。
- MCP metadata/challenge、resource/client/scope/audience 双向隔离、错误 Host/Origin、请求体/
  限流/并发/超时、低基数 telemetry、能力清单漂移、租户/角色/字段越权和安全错误；
- 真实 OIDC Authorization Code + S256 PKCE + RFC 8707 smoke，以及官方 server conformance。
