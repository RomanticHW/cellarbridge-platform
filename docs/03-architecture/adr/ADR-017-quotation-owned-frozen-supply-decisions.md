...
1. Planning 发布的不可变 `SupplyDecisionSnapshot` 是上下文间语言；Quotation 只通过公共 `TradePlanningService` 获取，并在自身 Revision 独立拥有副本。状态以 `UNDECIDED`、`FROZEN`、`LEGACY_REEVALUATION_REQUIRED` 表达；历史 Revision 不补造证据。
...
9. Quotation freeze 已通过 ADR-018 传播至 Trade Order；二者均不代表库存预占。
