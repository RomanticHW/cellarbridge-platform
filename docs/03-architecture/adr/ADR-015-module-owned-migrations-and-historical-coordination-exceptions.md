...
ownership manifest 全量覆盖磁盘 V2+，记录 version、file、owner Schemas、legacyException 和 SHA-256；只有 V8/V9 可引用 ADR-015 并声明上述精确 owner 集合，门禁双向校验覆盖、唯一性、历史集合、hash 与例外。严格 statement scanner 仅作用于 V10+：Schema-qualified DDL/DML/backfill 只能触及唯一 owner；procedural/dynamic SQL、`search_path`、注释、未知或未限定对象及无法可靠归属的结构 fail closed，同 owner 静态 trigger 允许。它是高信号检查，不宣称完整 SQL parser。
