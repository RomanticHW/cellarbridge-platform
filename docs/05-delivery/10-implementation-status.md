...
| Exception Center and recovery | Implemented in review | V19 去重 case/occurrence/history/recovery/work-item；库存失败、履约失败/逾期与技术投递检测；源状态验证的库存重试、履约重试/恢复和 publication 重放；权限/租户/掩码边界；OpenAPI 1.10/generated client；React queue/detail；PostgreSQL 与组件证据 |
...
当前基线已在 Quotation、Current/Legacy Event 和 Trade Order 中保存一致决策证据；Task 08 Inventory reservation 与 Task 09 Fulfillment orchestration 已可运行。Task 10 Exception Center 已形成完整纵向实现并处于 review：内部工作台展示去重案例、检测证据、责任人、恢复尝试和技术投递摘要；只有源模块返回可验证状态后才进入 `RESOLVED`，客户与 Buyer 边界不暴露内部异常证据。Task 10 合并与 CI 全绿前仍不标记 Available。
