...
| Authenticated read-only MCP and agent workflows  | Available                   | `/mcp` 上的 STATELESS Streamable HTTP；6 tools、3 resources、3 prompts；复用 OIDC、权限、租户与字段脱敏；`OperationsMcpApiIntegrationTest`、真实 OIDC smoke、官方 conformance 5/5 场景与 `docs/evidence/mcp/conformance-summary.md`                                 |
...
当前 `main` 在 v1.0.0 业务闭环之上增加了认证只读 MCP：智能客户端可以读取当前身份、供给、工作项、驾驶舱、业务时间线和审计数据，并使用 3 个受控 prompts 组织运营分析。该接口没有写工具，不安装模型提供商 SDK、RAG 或向量库，也不绕过既有权限与租户边界。已发布标签 `v1.0.0` 早于 Task 16，固定的 tag、release notes 和资产不包含 MCP。
