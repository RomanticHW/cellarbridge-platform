...
user journey/page specs → OpenAPI generated client → delivered feature code → corresponding E2E；dashboard 仍为 Planned。

### 30 秒简历描述

“设计并实现 CellarBridge，一套 Java 21/Spring Boot/React 的模块化 B2B 贸易协同平台。当前可运行核心覆盖多租户准入、供给检索、可解释报价/路径评估、客户决定、幂等报价转订单与本地可靠事件；PostgreSQL 原子库存预占已完成协议设计，完整可观测性属于后续计划。”
...
| 如何避免超卖？ | ADR-014 的 designed conditional SQL/savepoint 协议；实现与并发证据留 Task 08 |
| 事件丢了怎么办？ | 当前本地 publication/Inbox 的事务回滚、有限重试与终态测试；进程重启恢复、external outbox/reconcile 为后续证据 |
...
| React 有什么价值？ | 已交付流程的可操作 UI、query state、冲突与恢复状态 |
