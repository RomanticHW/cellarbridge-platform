...
| `INVENTORY_ALLOCATION_MISMATCH` | 409 | Allocation 单位或命令证据不匹配 |
| `INVENTORY_OPERATION_CONFLICT` | 409 | 预留操作与已有动作或原子余额冲突 |

事件终态错误另含 `QUOTATION_ACCEPTED_SUPPLY_DECISION_INVALID`、`QUOTATION_ACCEPTED_SUPPLY_DECISION_HASH_MISMATCH`、`TRADE_ORDER_CREATED_COMMERCIAL_HASH_MISMATCH` 与 `ORDER_SUPPLY_DECISION_CONFLICT`；它们不是 HTTP 错误码。
...
| `PAYMENT_REVERSAL_EXCEEDS_AVAILABLE` | 409 | 冲正金额超过该付款尚未冲正的余额 |
| `RESOURCE_VERSION_CONFLICT` | 412 | 应收版本已变化，刷新后重试 |
| `IDEMPOTENCY_KEY_REUSED` | 409 | 同一幂等键绑定了不同请求 |
