...
V21 的 `audit_reporting.audit_entry` 只由版本化可靠事件投影写入，数据库 trigger 拒绝普通
update/delete。事件 payload 不直接复制；actor、状态、reason、摘要、correlation/causation 采用
显式 allow-list。`/audit/entries` 使用 tenant-bound HMAC cursor 和稳定 `(occurredAt,id)` 排序。
该 hash 是完整性辅助证据，不声明法律不可篡改认证；导出、水印和生产 retention 仍不在当前范围。
